...
@Command({ name: 'execute-calculate-placment', options: { isDefault: false } })
export class CalculateReportPlacementCommand extends CommandRunner {
  constructor(
  ...
  async run(passedParams: string[]) {
    ...
    const queryResult = await this.reportService.executePlacement(caldate);
