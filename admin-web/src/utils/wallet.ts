...
export class Wallet {
    ...
    getPubKey(): string {
        return bytesToHex(secp256k1.getPublicKey(hexToBytes(this.account.privateKey), true));
    }
    ...
    getSharedSecret(pubKey: string): string {
        ...
        return bytesToHex(secp256k1.getSharedSecret(hexToBytes(this.account.privateKey),pubKey)).substring(2);
    }
}
